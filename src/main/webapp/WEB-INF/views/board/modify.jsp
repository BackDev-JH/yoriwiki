...
<script type="text/javascript">
   ...
   /*  	if(frm.cok_video_url.value == '' && 
    	   document.getElementById('video').src == '' && 
    	  (document.getElementById('divMainPhotoHolder').src == "https://recipe1.ezmember.co.kr/img/pic_none4.gif" 
    			  || 
    	   document.getElementById('divMainPhotoHolder').src == "https://img.youtube.com/vi/undefined/0.jpg" )
    		) {
    		check = true;
    	} */
    	...
    	for(var i = 1; i <= children.length; i++) {
    		...
    		if(document.querySelector('#stepPhotoHolder_' +i).src == 'https://recipe1.ezmember.co.kr/img/pic_none2.gif') {
        		check = true;
        		break;
        	}
    	}
	...
	function delWorkPhoto(step) {
		 ...
		 if($('#work_photo_' + step).val() != null && $('#work_photo_' + step).val() != "") {
			 $("#del_work_photo_" + step).val('Y');	 
		 } else {
			 $("#del_work_photo_" + step).val('');
	...
	function deleteVideoFile() {
		...
		document.getElementById('del_video_photo').value = 'Y';
		document.getElementById('new_video_photo').value = "";
    ...
    function delStep(step) {
        ...
        if(document.getElementById('stepItemNum_' + step).value != null && document.getElementById('stepItemNum_' + step).value != "") {
        	document.getElementById('del_step_photo_list').value = 
        		  document.getElementById('del_step_photo_list').value 
        		+ document.getElementById('stepItemNum_' + step).value 
        		+ ",";
        }
    }
    ...
    function addStep() {
    	...
    	stepElement.querySelector("input[name='step_no[]']").value = '';
    	stepElement.children[4].name = 'stepItemNum_' + stepCnt;
    	stepElement.children[4].value = '';
    	...
    	document.getElementById('divStepArea').append(stepElement);
    	
    	$('#q_step_file_' + stepCnt).change(() => {
	...
	$(document).ready(function() {
		...
		$('#q_video_file').change( () => {
			...
			var video = $('#video')[0];
			uploadCheck(input, 'video');
			
			if(inputFile.files.size != 0) {
			    var videourl = URL.createObjectURL(inputFile.files[0]);
			    $('#del_video_photo').val('Y');
	...
	function loadEvent() {
		$("#divStepArea [id^=divStepItem_]").each(function (idx, obj) {
			 $("#divStepArea [id=divStepItem_" + (idx + 1) + "]")
	  	     .mouseover( function () {
	        	  $(this).find('.step_btn').show();
	      	} )
	      	 .mouseout( function () {
...
<script type="text/javascript">
 	window.onload = function() {
 		$('#category').val(${bVo.category}).prop("selected", true);
 		$('#time').val(${bVo.time}).prop("selected", true);
 		$('#diff').val(${bVo.diff}).prop("selected", true);
 		addEvent();
 		loadEvent();
 		
 		var valueBox = document.getElementById('mySingleFieldTags');
 		var str = '';
 		
 		$('.tagit.ui-widget.ui-widget-content.ui-corner-all').children().each(function (idx, obj) {
 			var at = obj.querySelector('.tagit-close');
 			if(at != null) {
	 			str += obj.querySelector('.tagit-label').innerText + ',';
	 			at.addEventListener('click', function(e) {
					var parent = this.parentNode;
					var tempStr = parent.children[0].innerText;
					var arr = document.getElementById('mySingleFieldTags').value.split(',');
		...
		if(document.getElementById('video').style.display != 'none') {
			document.getElementById('videoPhotoHolder').readOnly = true;
			document.getElementById('videoBtn').disabled='disabled';
			document.getElementById('cok_video_url').readOnly = true;
		} else {
			document.getElementById('videoPhotoHolder').readOnly = false;
		...
		if($('#cok_video_url').val() != null && $('#cok_video_url').val() != '') {
			document.getElementById('cok_video_url').readOnly = true;
			$('#cok_video_url').focus();
			$('#title').focus();
		}
...
.tagit {
	...
	<form name="insFrm" id="insFrm" method="post"
		...
		<input type="hidden" name="q_mode" id="q_mode" value="modify">
		<input type="hidden" name="cok_sq_board" id="cok_sq_board" value="">
		<input type="hidden" name="newUploadFiles" value=""> <input
			type="hidden" name="boardNum" id="boardNum" value="${bVo.boardNum}">
...
ul.tagit input[type="text"] {
	...
	height: 30px;
		...
		<div class="container recipe_regi">
			<div class="regi_center">
				...
				<div class="cont_box pad_l_60">
					<div id="divMainPhotoUpload" class="cont_pic2">
						...
						<div style="position: absolute; left: -3000px">
							...
							<input type="file" name="q_main_file" id="q_main_file"
								file_gubun="main" accept="${picture.allowedExtensions}"
						...
						<div id="divMainPhotoBox" is_over="0">
							<a id="btnDelMainPhoto" href="javascript:delWorkPhoto(1)"
								class="pic_del" style="display: none;"></a>
							<c:choose>
								<c:when test="${fn:length(lbmVo_work) > 0}">
									<img id="divMainPhotoHolder" onclick="browseWorkFile(1)"
										src="${pageContext.request.contextPath}/upload/${lbmVo_work[0].fileName}"
					...
					<div class="cont_line pad_b_25">
						...
						<textarea name="introduce" id="introduce"
							...
							style="height: 100px; width: 610px; resize: none;">${bVo.introduce}</textarea>
					</div>

					<div class="cont_line pad_b_25">
						<p class="cont_tit4">동영상</p>
						<input type="hidden" name="video_photo" id="video_photo"
							value="${lbmVo_video[0].mediaNum }"> <input type="hidden"
							name="new_video_photo" id="new_video_photo" value=""> <input
							type="hidden" name="del_video_photo" id="del_video_photo"
							value=""> <input type="hidden" name="src" id="src"
							value="">
						...
						<textarea name="cok_video_url" id="cok_video_url"
							...
							style="height: 100px; width: 380px; resize: none;">${bVo.src}</textarea>

						<div style="position: absolute; left: -3000px">
							<input type="file" name="q_video_file" id="q_video_file"
								file_gubun="video"
								accept="${video.allowedExtensions}"
								style="width: 0px; height: 0px; font-size: 0px;" text="">
						</div>
						<div id="divVideoPhotoBox" is_over="0" class="thumb_m">
							<c:choose>
								<c:when test="${fn:length(lbmVo_video) > 0}">
									<video id="video"
										src="${pageContext.request.contextPath}/upload/${lbmVo_video[0].fileName}"
										style="display:; width: 177px; height: 100px; vertical-align: middle;" autoplay></video>
									<img id="videoPhotoHolder"
										src="https://recipe1.ezmember.co.kr/img/pic_none5.gif"
										style="display: none; width: 177px; height: 100px;">
								</c:when>
								<c:otherwise>
									<video id="video"
										style="display: none; width: 177px; height: 100px; vertical-align: middle;"></video>
									<c:if test="${bVo.src != null && bVo.src != ''}">
										<c:set var="url" value="${fn:split(bVo.src,'v=')[1]}" />
										<img id="videoPhotoHolder"
							...
							<!-- <video id="video" style="display: none; width: 177px; height: 100px; vertical-align: middle;"></video>
				...
				<div class="cont_box pad_l_60">
					...
					<span class="guide mag_b_15"><b>요리의 맛이 좌우될 수 있는 중요한 부분은
						...
						▷ 꿀이 없는 경우, 설탕 1스푼으로 대체 가능합니다. </span> <input type="hidden"
						name="del_step_photo_list" id="del_step_photo_list" value="">
					<div id="divStepArea" class="ui-sortable">
						<c:forEach var="item" items="${lbmVo_step}">
							<c:set var="step" value="${item.step}"></c:set>
							<div id="divStepItem_${item.step}" class="step">
								<p id="divStepNum_${item.step}"
									class="cont_tit2_1 ui-sortable-handle" style="cursor: pointer"
									data-original-title="" title="">Step${item.step}</p>
								<div id="divStepText_${item.step}" style="display: inline-block">
									<textarea name="step_text[]" id="step_text_${item.step}"
										class="form-control step_cont"
										placeholder="예) 소고기는 기름기를 떼어내고 적당한 크기로 썰어주세요."
										style="height: 160px; width: 430px; resize: none;">${item.content}</textarea>
								</div>
								<div id="divStepUpload_${item.step}"
									style="display: inline-block">
									<input type="hidden" name="step_no[]" id="step_no_${item.step}"
										value="${item.mediaNum }"> <input type="hidden"
										name="step_photo[]" id="step_photo_${item.step}"
										value="${item.fileName }"> <input type="hidden"
										name="new_step_photo[]" id="new_step_photo_${item.step}"
										value=""> <input type="hidden" name="del_step_photo[]"
										id="del_step_photo_${item.step}" value="">
									<div style="position: absolute; left: -3000px">
										<input type="file" id="q_step_file_${item.step}"
											name="q_step_file_${item.step}" file_gubun="step"
											accept="${picture.allowedExtensions}"
											style="display:; width: 0px; height: 0px; font-size: 0px;"
											text="">
									</div>
									<div id="divStepPhotoBox_${item.step}" is_over="0">
										<img id="stepPhotoHolder_${item.step}"
											onclick="browseStepFile(${item.step})"
											src="${pageContext.request.contextPath}/upload/${item.fileName}"
											width="160" height="160" style="cursor: pointer">
									</div>
								</div>
								<div id="divStepBtn_${item.step}" class="step_btn"
									style="display: none;">
									<a href="javascript:addStep(${item.step})"> <span
										class="glyphicon glyphicon-plus"></span>
									</a> <a href="javascript:delStep(${item.step})"> <span
										class="glyphicon glyphicon-remove"></span></a>
								</div>
								<input type="hidden" name="stepItemNum_${item.step}"
									id="stepItemNum_${item.step}" value="${item.mediaNum}">
							</div>
						</c:forEach>
					</div>
							...
							<div id="divStepUpload_STEP" style="display: inline-block">
								...
								<div style="position: absolute; left: -3000px">
									<input type="file" name="q_step_file_STEP"
										id="q_step_file_STEP" file_gubun="step" accept="${picture.allowedExtensions}"
										style="display:; width: 0px; height: 0px; font-size: 0px;"
										text="">
								</div>
					...
					<p class="cont_tit4">
						...
						<input type="file" name="file" id="multifile_2" file_gubun="work"
							style="display: none;" multiple="" accept="${picture.allowedExtensions}"> <br>
					</p>
					<div id="divWorkArea" style="display: inline-block"
						...
						<div id="divWorkUpload_1" class="complete_pic">
							<input type="hidden" name="work_photo[]" id="work_photo_1"
								value="${lbmVo_work[0].mediaNum }"> <input type="hidden"
								name="work_fileName[]" id="work_fileName[]"
								value="${lbmVo_work[0].fileName}"> <input type="hidden"
								name="new_work_photo[]" id="new_work_photo_1" value="">
							<input type="hidden" name="del_work_photo[]"
						...
						<div id="divWorkUpload_2" class="complete_pic">
							<input type="hidden" name="work_photo[]" id="work_photo_2"
								value="${lbmVo_work[1].mediaNum }"> <input type="hidden"
								name="work_fileName[]" id="work_fileName[]"
								value="${lbmVo_work[1].fileName}"> <input type="hidden"
								name="new_work_photo[]" id="new_work_photo_2" value="">
							<input type="hidden" name="del_work_photo[]"
							...
							<div id="divWorkPhotoBox_2" is_over="0">
								<a href="javascript:delWorkPhoto(2)" class="pic_del"
									style="display: none"></a>
								<c:choose>
									<c:when test="${fn:length(lbmVo_work) > 1}">
										<img id="workPhotoHolder_2" onclick="browseWorkFile(2)"
											src="${pageContext.request.contextPath}/upload/${lbmVo_work[1].fileName}"
											alt="No Image"
											style="width: 140px; height: 140px; cursor: pointer">
									</c:when>
									<c:otherwise>
						...
						<div id="divWorkUpload_3" class="complete_pic">
							<input type="hidden" name="work_photo[]" id="work_photo_3"
								value="${lbmVo_work[2].mediaNum }"> <input type="hidden"
								name="work_fileName[]" id="work_fileName[]"
								value="${lbmVo_work[2].fileName}"> <input type="hidden"
								name="new_work_photo[]" id="new_work_photo_3" value="">
							<input type="hidden" name="del_work_photo[]"
							...
							<div id="divWorkPhotoBox_3" is_over="0">
								<a href="javascript:delWorkPhoto(3)" class="pic_del"
									style="display: none"></a>
								<c:choose>
									<c:when test="${fn:length(lbmVo_work) > 2}">
										<img id="workPhotoHolder_3" onclick="browseWorkFile(3)"
											src="${pageContext.request.contextPath}/upload/${lbmVo_work[2].fileName}"
											alt="No Image"
											style="width: 140px; height: 140px; cursor: pointer">
									</c:when>
									<c:otherwise>
						...
						<div id="divWorkUpload_4" class="complete_pic">
							<input type="hidden" name="work_photo[]" id="work_photo_4"
								value="${lbmVo_work[3].mediaNum }"> <input type="hidden"
								name="work_fileName[]" id="work_fileName[]"
								value="${lbmVo_work[3].fileName}"> <input type="hidden"
								name="new_work_photo[]" id="new_work_photo_4" value="">
							<input type="hidden" name="del_work_photo[]"
							...
							<div id="divWorkPhotoBox_4" is_over="0">
								<a href="javascript:delWorkPhoto(4)" class="pic_del"
									style="display: none"></a>
								<c:choose>
									<c:when test="${fn:length(lbmVo_work) > 3}">
										<img id="workPhotoHolder_4" onclick="browseWorkFile(4)"
											src="${pageContext.request.contextPath}/upload/${lbmVo_work[3].fileName}" alt="No Image" style="width: 140px; height: 140px; cursor: pointer">
									</c:when>
									<c:otherwise>
										<img id="workPhotoHolder_4" onclick="browseWorkFile(4)"
											src="https://recipe1.ezmember.co.kr/img/pic_none3.gif"
				...
				<div class="cont_box pad_l_60">
					...
					<ul class="tagit ui-widget ui-widget-content ui-corner-all">
						<c:set var="TagList" value="${fn:split(tagStr, ',')}" />
						<c:forEach var="tag" items="${TagList}">
							<c:if test="${tag != ''}">
								<li
									class="tagit-choice ui-widget-content ui-state-default ui-corner-all tagit-choice-editable">
									<span class="tagit-label">${tag}</span> <a class="tagit-close">
										<span class="text-icon">x</span> <span
						...
						<li class="tagit-new" style="width: 99%;">
						<input type="text"
			...
			<!--/regi_center-->
			<div class="regi_btm">
				<button type="button" onclick="doSubmit('modify')" class="btn-lg btn-primary">수정</button>
